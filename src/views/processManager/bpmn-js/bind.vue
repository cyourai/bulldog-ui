<template>
  ...
  <div v-loading="loading"
    ...
    <time-picker v-loading="expectTimeLoading"
                 element-loading-spinner="el-icon-loading"
                 class="time-picker"
                 :propMinute="propMinute"
  ...
  methods: {
    ...
    refreshBindInfo() {
      ...
      if (isNotEmpty(this.modelId) && isNotEmpty(this.taskId)) {
        ...
        this.expectTimeLoading = true
        selectExpectTime({ expectTimeBusinessCode: this.taskId }).then(result => {
          const expectTime = result.data.expectTimeList.length === 0 ? 0
            : result.data.expectTimeList[0].expectTimeLength
          this.expectTimeCode = result.data.expectTimeList.length === 0 ? '' : result.data.expectTimeList[0].expectTimeCode
          this.expectTime = result.data.expectTimeList.length === 0 ? 'insert' : 'update'
          this.propMinute = (parseInt(expectTime / 60)).toString()
          this.propSecond = (expectTime % 60).toString()
        }).finally(() => {
          this.expectTimeLoading = false
        })
      }
    },
    // 打开穿梭框
    ...
    // 提交预计时间
    timeSubmit(time) {
      if (this.expectTime === 'insert') { // 如果是新增
        const data = {
          expectTimeBusinessCode: this.taskId,
          expectTimeLength: time,
          expectTimeIsActive: 1,
          expectTimeDescribe: ''
        }
        insertExpectTime(data).then(() => {
          this.expectTime === 'update'
        })
      } else { // 如果是更新
        const data = {
          expectTimeCode: this.expectTimeCode,
          expectTimeBusinessCode: this.taskId,
          expectTimeLength: time,
          expectTimeIsActive: 1,
          expectTimeDescribe: ''
        ...
        this.expectTimeLoading = true
        updateExpectTime(data).then(result => {
        }).finally(() => {
          this.expectTimeLoading = false
        })
      }
