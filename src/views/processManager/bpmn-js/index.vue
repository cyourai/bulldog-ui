    ...
    <div class="info"
      ...
      <div>
        <el-button type="warning"
                   plain
                   @click="zoomOut"
                   :disabled="loading">查看全景图</el-button>
        <el-button type="success"
                   plain
                   @click="zoomIn"
                   :disabled="loading">查看近景图</el-button>
      </div>
    </div>
    <!--控制面板-->
    <div class="panel"
...
// 业务相关
import { getByModelId, updateModel, deployByModelId } from '@/api/process/process'
// 绑定面板组件
...
export default {
  ...
  methods: {
    ...
    clickTaskHandler(node) {
      // 浏览模式隐藏左侧工具栏
      if (this.isView && node.element.type !== 'bpmn:Process') {
        document.getElementsByClassName('djs-context-pad')[0].style.display = 'none'
      }
      // console.log(node)
    ...
    zoomIn() {
      if (this.taskType === 'bpmn:CallActivity') {
        ...
        request({
          ...
          // this.$emit('modelId', result.data.modelId)
          window.location.href = `#/process/modelId/${result.data.modelId}`
          window.location.reload()
        }).catch(error => {
          console.error(error)
